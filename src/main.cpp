...
int main(int argc, char **argv) {
  ...
  // Note that in the section of associationTest, we will rescale the X to original genotype data
  if (params.associationTest) {
    cout << endl << "***** Compute association statistics and write result *****" << endl << endl;

    lmmcpu.calCalibrationFactor(phenodbl[0].data(), params.samplesCalibrationFactor);
    lmmcpu.computeStatistics(params.outputFile);
  }
